...
## 关于变量复制与参数的传递
 ...
 ```
>>>>>
  关于 `基本类型值` 不可改变, 此处举例不够典型, 可以再适当修改
<<<<<
...
>>>>>
  可以为关键词加一个 `粗体` (↓ 下面的 `基本类型` 与 `引用类型` 是不是变粗了)
<<<<<
...
>>>>>
  总结 : 
  - blog应该更多的是 `记录`, `探讨` 的话可以留给我们内部来深入探讨或者等待你blog的访问量上来的, 否则无用
  - blog应该更多的是 `新的/经验` 
  - blog一定要有一个总结, 放文章头或者文章尾部, blog目前你这个阶段更多的是给自己看的; 回忆一下你高中时的笔记方法, 是不是要有一个总结, 否则怎么快速的回忆起你这篇文章的核心思想和使用方法 ?
<<<<<
